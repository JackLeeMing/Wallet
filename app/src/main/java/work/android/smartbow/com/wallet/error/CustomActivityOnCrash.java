...
public final class CustomActivityOnCrash {
    ...
    private static final String EXTRA_RESTART_ACTIVITY_CLASS = "cat.ereza.customactivityoncrash" +
            ".EXTRA_RESTART_ACTIVITY_CLASS";
    private static final String EXTRA_SHOW_ERROR_DETAILS = "cat.ereza.customactivityoncrash" +
            ".EXTRA_SHOW_ERROR_DETAILS";
    private static final String EXTRA_STACK_TRACE = "cat.ereza.customactivityoncrash" +
            ".EXTRA_STACK_TRACE";
    private static final String EXTRA_IMAGE_DRAWABLE_ID = "cat.ereza.customactivityoncrash" +
            ".EXTRA_IMAGE_DRAWABLE_ID";

    //General constants
    private final static String TAG = "CustomActivityOnCrash";
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "cat.ereza.customactivityoncrash" +
            ".ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "cat.ereza.customactivityoncrash" +
            ".RESTART";
    ...
    private static int defaultErrorActivityDrawableId = R.drawable
            .customactivityoncrash_error_image;
        ...
        try {
            if (context == null) {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                    Log.w(TAG, "CustomActivityOnCrash will be installed, but may not be reliable " +
                            "in API lower than 14");
                }

                //INSTALL!
                Thread.UncaughtExceptionHandler oldHandler = Thread
                        .getDefaultUncaughtExceptionHandler();

                if (oldHandler != null && oldHandler.getClass().getName().startsWith
                    ...
                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith
                        ...
                        Log.e(TAG, "IMPORTANT WARNING! You already have an " +
                                "UncaughtExceptionHandler, are you sure this is correct? If you " +
                                "use ACRA, Crashlytics or similar libraries, you must initialize " +
                                "them AFTER CustomActivityOnCrash! Installing anyway, but your " +
                                "original handler will not be called.");
                    }

                    application = (Application) context.getApplicationContext();

                    //We define a default exception handler that does what we want so it can be
                    // called from Crashlytics/ACRA
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler
                            () {
                        @Override
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            Log.e(TAG, "App has crashed, executing CustomActivityOnCrash's " +
                            ...
                            if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                                Log.e(TAG, "Your application class or your error activity have " +
                                        "crashed, the custom activity will not be launched!");
                            } else {
                                if (launchErrorActivityWhenInBackground || !isInBackground) {
                                    ...
                                    //Reduce data to 128KB so we don't get a
                                    // TransactionTooLargeException when sending the intent.
                                    //The limit is 1MB on Android but some devices seem to have
                                    // it lower.
                                    //See: http://developer.android
                                    // .com/reference/android/os/TransactionTooLargeException.html
                                    //And: http://stackoverflow
                                    // .com/questions/11451393/what-to-do-on-transactiontoolargeexception#comment46697371_12809171
                                    ...
                                    if (enableAppRestart && restartActivityClass == null) {
                                        //We can set the restartActivityClass because the app
                                        // will terminate right now,
                                        //and when relaunched, will be null again by default.
                                        restartActivityClass = guessRestartActivityClass
                                                (application);
                                    } else if (!enableAppRestart) {
                                        //In case someone sets the activity and then decides to
                                        // not restart
                                        restartActivityClass = null;
                                    }

                                    intent.putExtra(EXTRA_STACK_TRACE, stackTraceString);
                                    intent.putExtra(EXTRA_RESTART_ACTIVITY_CLASS,
                                            restartActivityClass);
                                    intent.putExtra(EXTRA_SHOW_ERROR_DETAILS, showErrorDetails);
                                    intent.putExtra(EXTRA_IMAGE_DRAWABLE_ID,
                            ...
                            if (lastActivity != null) {
                                //We finish the activity, this solves a bug which causes infinite
                                // recursion.
            ...
            Log.e(TAG, "An unknown error occurred while installing CustomActivityOnCrash, it may " +
                            "not have been properly initialized. Please report this as a bug if needed.",
                    t);
        }
    }
    ...
    public static int getDefaultErrorActivityDrawableIdFromIntent(Intent intent) {
        return intent.getIntExtra(CustomActivityOnCrash.EXTRA_IMAGE_DRAWABLE_ID, R.drawable
                .customactivityoncrash_error_image);
    }
    ...
    /**
     * Given an Intent, returns several error details including the stack trace extra from the
     * intent.
     *
    ...
    public static String getAllErrorDetailsFromIntent(Context context, Intent intent) {
        ...
        errorDetails += "System version: "+ Build.VERSION.RELEASE+" \n";
        errorDetails += "System version int int: "+ Build.VERSION.SDK_INT+" \n";
        errorDetails += "System base_os: "+ Build.VERSION.BASE_OS+" \n";
    ...
    public static Class<? extends Activity> getRestartActivityClassFromIntent(Intent intent) {
        Serializable serializedClass = intent.getSerializableExtra(CustomActivityOnCrash
                .EXTRA_RESTART_ACTIVITY_CLASS);
    ...
    public static void setLaunchErrorActivityWhenInBackground(boolean launchErrorActivityWhenInBackground) {
        CustomActivityOnCrash.launchErrorActivityWhenInBackground =
                launchErrorActivityWhenInBackground;
    }
    ...
    /**
     * INTERNAL method that checks if the stack trace that just crashed is conflictive. This is
     ...
     * @return true if this stack trace is conflictive and the activity must not be launched,
     * false otherwise
     */
    private static boolean isStackTraceLikelyConflictive(Throwable throwable, Class<? extends
            Activity> activityClass) {
        do {
            StackTraceElement[] stackTrace = throwable.getStackTrace();
            for (StackTraceElement element : stackTrace) {
                if ((element.getClassName().equals("android.app.ActivityThread") && element
                        .getMethodName().equals("handleBindApplication")) || element.getClassName
    ...
    /**
     * INTERNAL method that returns the version name of the current app, or null if unable to
     * determine it.
     *
     * @param context A valid context. Must not be null.
     ...
     * android:name="cat.ereza.customactivityoncrash.RESTART" />,
    ...
    private static Class<? extends Activity> guessRestartActivityClass(Context context) {
        ...
        resolvedActivityClass = CustomActivityOnCrash.getRestartActivityClassWithIntentFilter
                (context);
    ...
    private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(Context context) {
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ...
            try {
                ...
                Log.e(TAG, "Failed when resolving the restart activity class via intent filter, " +
    ...
    private static Class<? extends Activity> getLauncherActivity(Context context) {
        Intent intent = context.getPackageManager().getLaunchIntentForPackage(context
                .getPackageName());
        if (intent != null) {
            try {
                return (Class<? extends Activity>) Class.forName(intent.getComponent()
                        .getClassName());
            } catch (ClassNotFoundException e) {
                //Should not happen, print it to the log!
                Log.e(TAG, "Failed when resolving the restart activity class via " +
                        "getLaunchIntentForPackage, stack trace follows!", e);
            }
        }
     ...
     * It will first get activities from the AndroidManifest with intent filter <action
     * android:name="cat.ereza.customactivityoncrash.ERROR" />,
    ...
    private static Class<? extends Activity> guessErrorActivityClass(Context context) {
        ...
        //If action is defined, use that
        resolvedActivityClass = CustomActivityOnCrash.getErrorActivityClassWithIntentFilter
                (context);
    ...
    private static Class<? extends Activity> getErrorActivityClassWithIntentFilter(Context context) {
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ...
            try {
                ...
                //Should not happen, print it to the log!
                Log.e(TAG, "Failed when resolving the error activity class via intent filter, " +
                        "stack trace follows!", e);
            }
        }
